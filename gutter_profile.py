...
for ax, fname in zip(axes, files):
    ...
    ax.axvline(x=w // 2, color="red", linestyle="--", linewidth=0.8,
               label="midpoint")
    # mark the darkest column in the central 20%
    lo = int(w * 0.4)
    hi = int(w * 0.6)
    gutter_x = lo + np.argmin(profile[lo:hi])
    ax.axvline(x=gutter_x, color="green", linestyle="--", linewidth=0.8,
               label=f"min@{gutter_x}")
    ax.set_ylabel(fname.replace("cam1753-page-", "").replace(".jpg", ""),
                  fontsize=8, rotation=0, labelpad=40)
    ax.set_xlim(0, max(4700, w))
    ax.tick_params(labelsize=7)
    ax.legend(fontsize=7, loc="upper right")

fig.suptitle("Column-mean brightness profiles (grayscale)\n"
