...
def read_in(uxlc, path_to_lci_recs):
    ...
    lcirs = _get_lcirs_at_high_resolution(path_to_lci_recs)
...
def read_lci_recs_dot_json(path_to_lci_recs):
    """Read in lci_recs.json, raw"""
    with open(path_to_lci_recs, encoding="utf-8") as lci_recs_json_in_fp:
        return json.load(lci_recs_json_in_fp)


def _get_lcirs_at_high_resolution(path_to_lci_recs):
    lci_recs_dot_json = read_lci_recs_dot_json(path_to_lci_recs)
