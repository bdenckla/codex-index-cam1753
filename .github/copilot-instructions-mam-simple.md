...
# How to Read MAM-simple

> **Canonical documentation:** The MAM-simple repo itself has a human-readable version of this guide at `..\..\GitRepos\MAM-simple\doc\reading-mam-simple.md`. This file is a Copilot-oriented copy with local path references.

## What is MAM-simple?

MAM-simple is an XML format containing **MAM (Miqra According to the Masorah)** — the Hebrew Bible text. The source of truth is the [MAM Google Sheet](https://docs.google.com/spreadsheets/d/1mkQyj6by1AtBUabpbaxaZq9Z2X3pX8ZpwG91ZCSOEYs/edit#gid=920165745). MAM-simple is one of several derived formats; others include [MAM-parsed](https://github.com/bdenckla/MAM-parsed) and [MAM for Sefaria](https://github.com/bdenckla/MAM-for-Sefaria).

**Repository:** `..\..\GitRepos\MAM-simple` (or [github.com/bdenckla/MAM-simple](https://github.com/bdenckla/MAM-simple))
...
tree = ET.parse('path/to/MAM-simple/out/xml-vtrad-mam/Job.xml')
root = tree.getroot()
...
### Handler-based approach (used by MAM-simple py-example)

The `py-example/` in the MAM-simple repo demonstrates a more sophisticated recursive handler pattern:
...
- **MAM-for-Sefaria** (`..\..\GitRepos\mam-for-sefaria`): CSV/HTML format generated from MAM-simple by `py-example/main_mam4sef.py`. Each row is `"Book Ch:Vs","<html_text>"`. This is a *derived* product — the XML is the source.
